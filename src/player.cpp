...
Player::Player(QObject* parent, ZoneMgr* zoneMgr, GuildMgr* guildMgr, const char* name)
  : QObject(parent, name),
	Spawn(),
	m_zoneMgr(zoneMgr),
	m_guildMgr(guildMgr)
{
	...
	connect(m_zoneMgr, SIGNAL(zoneChanged(const QString&)), this, SLOT(zoneChanged()));

	m_NPC = SPAWN_SELF;

	QString section = "Defaults";
	m_useAutoDetectedSettings = pSEQPrefs->getPrefBool("useAutoDetectedSettings", section, true);
	...
	m_conColorBases[tGraySpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tGraySpawn], "Player", QColor(140, 140, 140));
	m_conColorBases[tGreenSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tGreenSpawn], "Player", QColor(0, 95, 0));
	m_conColorBases[tCyanSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tCyanSpawn], "Player", QColor(0, 255, 255));
	m_conColorBases[tBlueSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tBlueSpawn], "Player", QColor(0, 0, 160));
	m_conColorBases[tEvenSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tEvenSpawn], "Player", QColor(255, 255, 255));
	m_conColorBases[tYellowSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tYellowSpawn], "Player", QColor(255, 255, 0));
	m_conColorBases[tRedSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tRedSpawn], "Player", QColor(127, 0, 0));
	m_conColorBases[tUnknownSpawn] = pSEQPrefs->getPrefColor(conColorBasePrefNames[tUnknownSpawn], "Player", gray);
...
void Player::player(const charProfileStruct* player)
{
	...
	// Position
	setPos((int16_t)lrintf(player->x), (int16_t)lrintf(player->y), (int16_t)lrintf(player->z), showeq_params->walkpathrecord, showeq_params->walkpathlength);
	setHeading((int8_t)lrintf(player->heading), 0);
	setDeltas(0,0,0);

	seqDebug("Player::backfill(): Pos (%f/%f/%f) Heading: %f", player->x, player->y, player->z, player->heading);
...
void Player::wearItem(const playerItemStruct* itemp)
{
	...
	if (item->equipSlot < 22)
	{
		...
		if (manaAdjust)
		{
			m_maxMana = calcMaxMana(m_maxINT, m_maxWIS, classVal(), level()) +  m_plusMana;
...
void Player::updateAltExp(const uint8_t* data)
{
	...
	*/
	...
	emit expAltChangedInt(m_currentAltExp, 0, 15000000);
	emit newAltExp(expIncrement, m_currentAltExp, altexp->altexp, 15000000, 15000000/330, m_currentAApts);
...
void Player::updateExp(const uint8_t* data)
{
	...
	emit newExp(expIncrement, realExp, exp->exp, m_minExp, m_maxExp, m_tickExp);
	emit expChangedInt(realExp, m_minExp, m_maxExp);

	if (m_freshKill)
	{
		emit expGained(m_lastSpawnKilledName, m_lastSpawnKilledLevel, expIncrement, m_zoneMgr->longZoneName());
	...
	{
		// Randomly blessed with xp?
		emit expGained("Unknown", 0, expIncrement, m_zoneMgr->longZoneName());
	}
...
void Player::updateLevel(const uint8_t* data)
{
	...
	emit newExp(expIncrement, m_currentExp, levelup->exp, m_minExp, m_maxExp, m_tickExp);
...
#pragma pack(0)
	...
	emit newSpeed((int)lrint(hypot(hypot((pupdate->deltaX*80), (pupdate->deltaY*80)), (pupdate->deltaZ*80))));
...
void Player::tradeSpellBookSlots(const uint8_t* data, size_t, uint8_t dir)
{
	...
	seqDebug("tradeSpellBookSlots(dir=%d): Swapping %d (%04x) with %d (%04x)",
		dir, tsb->slot1, m_spellBookSlots[tsb->slot1], tsb->slot2, m_spellBookSlots[tsb->slot2]);
...
bool Player::getStatValue(uint8_t stat, uint32_t& curValue, uint32_t& maxValue)
{
	bool valid = false;
	curValue = 0;
	maxValue = 0;
...
void Player::fillConTable()
{
	...
	{
		m_conTable[spawnLevel] = QColor(redColor, m_conColorBases[tRedSpawn].green(), m_conColorBases[tRedSpawn].blue());
		redColor -= redStep;
	}
...
{
	return (!m_useAutoDetectedSettings || m_useDefaults ? m_defaultName : m_name);
}
...
{
	return ((!m_useAutoDetectedSettings || m_useDefaults) ? m_defaultRace : m_race);
}

uint8_t Player::classVal() const
{
	return ((!m_useAutoDetectedSettings || m_useDefaults) ? m_defaultClass : m_class);
}
...
void Player::restorePlayerState()
{
	...
	if (keyFile.open(QIODevice::ReadOnly))
	{
		...
		{
			seqWarn("Failure loading %s: Bad player size!", (const char*)fileName);
			reset();
			clear();
			return;
		}
		...
		{
			seqWarn("Failure loading %s: Bad known skills!", (const char*)fileName);
			reset();
			clear();
			return;
		}
		...
		{
			seqWarn("Failure loading %s: Bad known langs!", (const char*)fileName);
			reset();
			clear();
			return;
		}
		...
		seqInfo("Restored PLAYER: %s (%s)!", (const char*)m_name, (const char*)m_lastName);
	}
	else
	{
