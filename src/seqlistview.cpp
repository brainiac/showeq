...
SEQListView::SEQListView(const QString prefName, QWidget* parent, const char* name, Qt::WFlags f)
...
int SEQListView::addColumn(const QString& label, int width)
{
	return addColumn(label, label, width);
}

int SEQListView::addColumn(const QString& label, const QString& preference, int width)
{
...
void SEQListView::setColumnVisible(int column, bool visible)
{
	...
	pSEQPrefs->setPrefBool(QString("Show") + columnName, preferenceName(), (width != 0));
