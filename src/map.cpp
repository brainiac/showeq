...
MapLabel::MapLabel( Map* map )
  : QLabel(map, "mapLabel", WStyle_Customize | WStyle_NoBorder | WStyle_Tool | WType_TopLevel
       | WType_Dialog | WX11BypassWM)
{
  m_Map = map;
  setMargin( 1 );
  setIndent( 0 );
...
void Map::paintMap (QPainter * p)
{
 ...
 //QPainter offscreen(&m_offscreen);
  ...
  if (m_showZoneSafePoint)
  {
    ...
    m_mapIcons->paintIcon(m_param, *p, 
  ...
  if(m_showInstanceLocationMarker && m_zoneMgr->dzID())
  {
     ...
     m_mapIcons->paintIcon(m_param,* p, m_mapIcons->icon(tIconTypeDynamicZoneLocation),
