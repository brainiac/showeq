...
static void displayGDBMError(const char* operation, const char* filename, const char *level = "Warning")
{
							 ...
							 bool update)
{
	...
	if ((dbf = gdbm_open((char*)(const char*)dbName, 0, GDBM_WRCREAT, 0644, 0)) != NULL)
	{
...
bool GDBMConvenience::IsEntryExist(QString dbName, Datum& key)
{
	...
	if ((dbf = gdbm_open((char*)(const char*)dbName, 0, GDBM_READER, 0644, 0)) != NULL)
	{
...
bool GDBMConvenience::Reorganize(QString dbName)
{
	...
	// attempt to open the database for write
	if ((dbf = gdbm_open((char*)(const char*)dbName, 0, GDBM_WRITER, 0644, 0)) != NULL)
	{
		// if successful, then attempt to insert or replace data under key
...
bool GDBMIterator::GetFirstKey(GDBMConvenience* gdbmc, QString dbName, Datum& key)
{
