...
void MapParameters::reAdjust()
{
	...
	m_screenCenter.setX((((iZMaxX + xoff) * m_screenLength.width()) / m_zoomMapLength.width()));
	m_screenCenter.setY((((iZMaxY + yoff) * m_screenLength.height()) / m_zoomMapLength.height()));

	// calculate screen bounds
	m_screenBounds = QRect(int((m_screenCenter.x() - m_screenLength.width()) * m_ratio), int((m_screenCenter.y() - m_screenLength.height()) * m_ratio),
		int(m_screenLength.width() * m_ratio), int(m_screenLength.height() * m_ratio));
...
//----------------------------------------------------------------------
// MapLineL
MapLineL::MapLineL() : MapCommon(), m_z(0), m_heightSet(false)
{
}
...
MapLocation::MapLocation(const QString& name, const QString& color, const QPoint& point)
  : MapCommon(name, color),
	MapPoint(point),
	m_heightSet(false)
{
...
MapLocation::MapLocation(const QString& name, const QString& color, const MapPoint& point)
  : MapCommon(name, color),
	MapPoint(point),
	m_heightSet(true)
{
...
MapLocation::MapLocation(const QString& name, const QString& color, int16_t x, int16_t y)
  : MapCommon(name, color),
	MapPoint(x, y, 0),
	m_heightSet(false)
{
...
MapLocation::MapLocation(const QString& name, const QString& color, int16_t x, int16_t y, int16_t z)
  : MapCommon(name, color),
	MapPoint(x, y, z),
	m_heightSet(true)
{
...
MapLocation::MapLocation(const QString& name, const QColor& color, int16_t x, int16_t y, int16_t z)
  : MapCommon(name, color),
	MapPoint(x, y, z),
	m_heightSet(true)
{
...
void MapData::loadMap(const QString& fileName, bool import)
{
	...
	if (!count)
	{
		seqWarn("Error, no fields in first line of map file '%s'", filename);
		return;
	}

	if (count < 2)
	{
		seqWarn("Error, too few fields in first line of map file '%s'", filename);
		return;
	}
	...
	for (++lit; lit != lines.end(); ++lit)
	{
		...
		case 'M':
			{
				...
				if (count < 3)
				{
					seqWarn("Error reading M line %d on map '%s'! %d is too few fields", filelines, filename, count);
					continue;
				}
				...
				if (linePoints < 2)
				{
					seqWarn("M Line %d in map '%s' only had %d points, not loading.", filelines, filename, linePoints);
					continue;
				}
		...
		case 'L':
			{
				...
				if (count < 3)
				{
					seqWarn("Error reading L line %d on map '%s'! %d is too few fields", filelines, filename, count);
					continue;
				}
				...
				if (linePoints < 2)
				{
					seqWarn("L Line %d in map '%s' only had %d points, not loading.", filelines, filename, linePoints);
					continue;
				}
		...
		case 'A':  //Creates aggro ranges.
			{
#ifdef DEBUGMAPLOAD
				seqWarn("A record  [%d] [%d fields]: %s", filelines, count, (const char*)*lit);
#endif
...
#ifdef DEBUGMAPLOAD
			seqDebug("H record [%d] [%d fields]: %s", filelines, count, (const char*)*lit);
#endif

			if (count < 1)
			{
				seqWarn("Line %d in map '%s' has an H record with too few fields (%d)!", filelines, filename, count);
				break;
			}
		...
		case 'Z':  // Quick and dirty ZEM implementation
			{
#ifdef DEBUGMAPLOAD
				seqWarn("Z record [%d] [%d fields]: %s", filelines, count, (const char*)*lit);
#endif

				if (count < 1)
				{
					seqWarn("Line %d in map '%s' has a Z record with too few fields (%d)!", filelines, filename, count);
					break;
				}
...
void MapData::loadSOEMap(const QString& fileName, bool import)
{
	...
	for (; lit != lines.end(); ++lit)
	{
		...
		case 'L':
			{
#ifdef DEBUGMAPLOAD
				seqDebug("L record  [%d] [%d fields]: %s", filelines, count, (const char*)*lit);
#endif

				if (count != 9)
				{
					seqWarn("Error reading L line %d on map '%s'! %d is an incorrect field count!", filelines, filename, count);
					continue;
				}
...
void MapData::saveMap(const QString& fileName) const
{
	...
	fprintf(fh, "%s,%s,%d,%d\n", (const char*)m_zoneLongName, (const char*)m_zoneShortName, m_size.width(), m_size.height());
	...
	for (currentLineL = mlit.toFirst(); currentLineL != NULL; currentLineL = ++mlit)
	{
		...
		fprintf(fh, "L,%s,%s,%d", (const char*)currentLineL->name(), (const char*)currentLineL->colorName(), currentLineL->size());
	...
	for (currentLineM = mmit.toFirst(); currentLineM; currentLineM = ++mmit)
	{
		// write out the start of the line info
		fprintf(fh, "M,%s,%s,%d", (const char*)currentLineM->name(), (const char*)currentLineM->colorName(), currentLineM->size());
		...
		for (i = 0; i < currentLineM->size(); i++)
		{
			...
			fprintf(fh, ",%d,%d,%d", curMPoint.x(), curMPoint.y(), curMPoint.z());
		}
		// terminate the line
		fprintf(fh, "\n");
	}
	...
	for (; lit.current(); ++lit)
	{
		...
		if (!currentLoc->heightSet())
			fprintf (fh, "P,%s,%s,%d,%d\n", (const char*)currentLoc->name(), (const char*)currentLoc->colorName(), currentLoc->x(), currentLoc->y());
		else
			fprintf (fh, "P,%s,%s,%d,%d,%d\n", (const char*)currentLoc->name(), (const char*)currentLoc->colorName(), currentLoc->x(), currentLoc->y(), currentLoc->z());
	}
...
void MapData::saveSOEMap(const QString& fileName) const
{
	...
	for (currentLineL = mlit.toFirst(); currentLineL != NULL; currentLineL = ++mlit)
	{
		...
		for (i = 1; i < currentLineL->size(); ++i)
		{
			...
			fprintf (fh, "L %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %d, %d, %d\n", -float(curQPoint.x()), -float(curQPoint.y()), z1,
				-float(lastQPoint.x()), -float(lastQPoint.y()), z1, r, g, b);
	...
	for (currentLoc = lit.toFirst(); currentLoc; currentLoc = ++lit)
	{
		...
		fprintf(fh, "P %.1f, %.1f, %.1f, %d, %d, %d, 3,  %s\n", -float(currentLoc->x()), -float(currentLoc->y()), float(currentLoc->z()),
			color.red(), color.green(), color.blue(), (const char*)name);
	}
#ifdef DEBUGMAP
	seqDebug("saveMap() - map '%s' saved with %d L lines, %d M lines, %d locations", filename,
		m_lLines.count(), m_mLines.count(), m_locations.count());
...
bool MapData::isAggro(const QString& name, uint16_t* range) const
{
	MapAggro* aggro;
	Q3PtrListIterator<MapAggro> ait(m_aggros);
	for (aggro = ait.toFirst(); aggro != NULL; aggro = ++ait)
	{
...
void MapData::addLocation(const QString& name, const QString& color, const QPoint& point)
{
...
void MapData::paintGrid(MapParameters& param, QPainter& p) const
{
	...
	for (int gx = (minX() / gridres) - 1; gx <= lastGrid; gx++)
	{
		// calculate position
		pos = gx * gridres;
		...
		if (param.showGridLines())
		{
			...
			p.drawLine(offsetPos, 0, offsetPos, param.screenLengthY());
		}
	...
	// start from the minimum position and increment to the last grid position
	for (int gy = (minY() / gridres) - 1; gy <= lastGrid; gy++)
	{
		// calculate position
		pos = gy * gridres;
...
void MapData::paintDepthFilteredLines(MapParameters& param, QPainter& p) const
{
	...
	for (currentLineL = mlit.toFirst(); currentLineL != NULL; currentLineL = ++mlit)
	{
		...
		if (currentLineL->heightSet() && !inRoom(param.playerHeadRoom(), param.playerFloorRoom(), currentLineL->z()))
	...
	for (currentLineM = mmit.toFirst(); currentLineM; currentLineM = ++mmit)
	{
		...
		// see if the starting position is in bounds
		lastInBounds = (inRect(screenBounds, curX, curY) && inRoom(param.playerHeadRoom(), param.playerFloorRoom(), curZ));
		...
		for (uint32_t i = 1; i < numPoints; i++)
		{
			...
			// determine if the current position is in bounds
			curInBounds = (inRect(screenBounds, curX, curY) && inRoom(param.playerHeadRoom(), param.playerFloorRoom(), curZ));
...
void MapData::paintLocations(MapParameters& param, QPainter& p) const
{
	...
	for (; lit.current(); ++lit)
	{
		...
		// draw the text
		p.drawText(param.calcXOffsetI(currentLoc->x()) - 2,param.calcYOffsetI(currentLoc->y()) - 2, currentLoc->name());
	}
}
...
bool MapData::paintMapImage(MapParameters& param, QPainter& p) const
{
	...
	double scaleX = (double(param.screenLength().width()) / double(m_image.width())) * double(param.zoom());
	double scaleY = (double(param.screenLength().height()) / double(m_image.height())) * double(param.zoom());
...
#ifndef QT3_SUPPORT
	if (m_lastParam.mapOptimizationMethod() != param.mapOptimizationMethod())
